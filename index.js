...
console.log(
  chalk.blue(
    figlet.textSync('Simple CLI', { horizontalLayout: 'full' })
  )
);
...
function getGithubToken(callback) {
  var prefs = new Preferences('Simple CLI');
  ...
  github.authorization.create({
    ...
    note: 'Simple CLI, the command-line tool for initalizing Git repository'
  }, function(err, res) {
    status.stop();
    if ( err ) {
      return callback( err );
...
function setupRepo(url, callback) {
  var status = new Spinner('Setting up your repository...');
  status.start();
